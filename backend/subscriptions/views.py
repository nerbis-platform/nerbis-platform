...
class MarketplaceContractViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            MarketplaceContract.objects.filter(customer=self.request.user)
            .select_related("service_plan", "service_plan__category", "order")
            .order_by("-created_at")
        )
    ...
    def cancel(self, request, pk=None):
        ...
        if contract.status == "cancelled":
            return Response({"error": "Este contrato ya está cancelado"}, status=status.HTTP_400_BAD_REQUEST)
...
def purchase_plan(request):
    ...
    existing_contract = MarketplaceContract.objects.filter(
        customer=request.user, service_plan=plan, status__in=["pending", "active"]
    ).first()

    if existing_contract:
        return Response(
            {"error": "Ya tienes un contrato activo o pendiente de este plan"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    return Response(
        {"plan": MarketplacePlanDetailSerializer(plan).data, "message": "Plan listo para agregar al carrito"},
