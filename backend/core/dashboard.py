...
def get_activity_log(request, limit=15, days=None):
    ...
    if not request.user.is_superuser:
        if hasattr(request.user, "tenant") and request.user.tenant:
            ...
            tenant_users = User.objects.filter(tenant=request.user.tenant).values_list("id", flat=True)
    ...
    activity_list = []
    for log in logs:
        action_info = action_map.get(log.action_flag, {"icon": "info", "color": "secondary", "label": "Acción"})
...
def get_low_stock_products(request, limit=10):
    ...
    low_stock_inventories = (
        Inventory.objects.filter(**tenant_filter, track_inventory=True, product__is_active=True)
        .filter(Q(stock=0) | Q(stock__lte=models.F("low_stock_threshold")))
    ...
    total_out_of_stock = Inventory.objects.filter(
        **tenant_filter, track_inventory=True, product__is_active=True, stock=0
    ).count()
...
def dashboard_callback(request, context):
    ...
    # Ventas del mes
    monthly_orders = orders_qs.filter(created_at__date__gte=start_of_month, status__in=["completed", "paid"])
    monthly_revenue = monthly_orders.aggregate(total=Sum("total"))["total"] or Decimal("0.00")
    monthly_orders_count = monthly_orders.count()

    # Ventas de hoy
    today_orders = orders_qs.filter(created_at__date=today, status__in=["completed", "paid"])
    today_revenue = today_orders.aggregate(total=Sum("total"))["total"] or Decimal("0.00")

    # Ventas de la semana
    weekly_orders = orders_qs.filter(created_at__date__gte=start_of_week, status__in=["completed", "paid"])
    ...
    # Citas de hoy
    today_appointments = appointments_qs.filter(start_datetime__date=today).count()

    # Citas pendientes (futuras pendientes o confirmadas)
    pending_appointments = appointments_qs.filter(
        start_datetime__gte=timezone.now(), status__in=["pending", "confirmed"]
    ).count()

    # Citas completadas este mes
    completed_appointments_month = appointments_qs.filter(
        start_datetime__date__gte=start_of_month, status="completed"
    ).count()
    ...
    new_users_month = users_qs.filter(created_at__date__gte=start_of_month, role="customer").count()

    # Total clientes activos
    total_customers = users_qs.filter(role="customer", is_active=True).count()
    ...
    sales_by_day = list(
        orders_qs.filter(created_at__date__gte=last_30_days, status__in=["completed", "paid"])
    ...
    sales_by_month = list(
        orders_qs.filter(created_at__date__gte=six_months_ago, status__in=["completed", "paid"])
    ...
    top_products = list(
        OrderItem.objects.filter(order__in=orders_qs.filter(status__in=["completed", "paid"]))
        .values("product_name")
    ...
    top_services = list(
        OrderServiceItem.objects.filter(order__in=orders_qs.filter(status__in=["completed", "paid"]))
        .values("service_name")
        .annotate(total_booked=Count("id"), total_revenue=Sum("price"))
    ...
    upcoming_appointments = appointments_qs.filter(
        start_datetime__gte=timezone.now(), status__in=["confirmed", "pending"]
...
def _get_staff_dashboard(request, staff_member, today, start_of_month):
    ...
    staff_appointments = Appointment.objects.filter(staff_member=staff_member)
    ...
    today_appointments = staff_appointments.filter(start_datetime__date=today)
    staff_today_appointments = today_appointments.count()

    # Citas completadas hoy
    staff_completed_today = today_appointments.filter(status="completed").count()

    # Clientes atendidos hoy (únicos)
    staff_total_customers_today = today_appointments.filter(status="completed").values("customer").distinct().count()
    ...
    # =====================
    staff_upcoming_appointments = (
        staff_appointments.filter(start_datetime__gte=timezone.now(), status__in=["confirmed", "pending"])
    ...
    # =====================
    staff_appointments_by_status = {
        "completed": staff_appointments.filter(start_datetime__date__gte=start_of_month, status="completed").count(),
        "confirmed": staff_appointments.filter(start_datetime__date__gte=start_of_month, status="confirmed").count(),
        "pending": staff_appointments.filter(start_datetime__date__gte=start_of_month, status="pending").count(),
        "cancelled": staff_appointments.filter(start_datetime__date__gte=start_of_month, status="cancelled").count(),
        "no_show": staff_appointments.filter(start_datetime__date__gte=start_of_month, status="no_show").count(),
    }
    ...
    staff_top_services = list(
        staff_appointments.filter(status="completed", start_datetime__date__gte=start_of_month)
    ...
    staff_assigned_services = (
        Service.objects.filter(assigned_staff=staff_member, is_active=True)
...
def badge_callback(request):
    ...
    pending_today = Appointment.objects.filter(
        **tenant_filter, start_datetime__date=today, status__in=["pending", "confirmed"]
    ).count()
...
def get_chart_data(request, period="month", group_by="day"):
    ...
    orders_qs = Order.objects.filter(
        **tenant_filter, created_at__date__gte=start_date, status__in=["completed", "paid"]
    )

    # Ventas agrupadas por período
    sales_data = list(
        orders_qs.annotate(period=trunc_func("created_at"))
        .values("period")
    ...
    # =====================
    products_revenue = OrderItem.objects.filter(order__in=orders_qs).aggregate(total=Sum("total_price"))[
        "total"
    ] or Decimal("0")

    services_revenue = OrderServiceItem.objects.filter(order__in=orders_qs).aggregate(total=Sum("price"))[
        "total"
    ] or Decimal("0")
    ...
    # DATOS DE CITAS
    # =====================
    appointments_qs = Appointment.objects.filter(**tenant_filter, start_datetime__date__gte=start_date)
    ...
    appointments_data = list(
        appointments_qs.annotate(period=trunc_func("start_datetime"))
        .values("period")
        .annotate(count=Count("id"))
        .order_by("period")
    )

    appointments_by_period = _fill_missing_dates(appointments_data, start_date, today, group_by, value_field="count")
