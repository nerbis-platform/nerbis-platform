...
class WebsiteTemplateAdmin(ModelAdmin):
    ...
    def display_template(self, obj):
        premium_badge = (
            mark_safe(
                ' <span style="background: #f59e0b; color: white; padding: 1px 6px; border-radius: 4px; font-size: 10px;">PREMIUM</span>'
            )
            if obj.is_premium
            else ""
        )
        return format_html("<strong>{}</strong>{}", obj.name, premium_badge)
...
class WebsiteConfigAdmin(ModelAdmin):
    ...
    def display_url(self, obj):
        ...
        if obj.is_published:
            return format_html(
                '<a href="{}" target="_blank" style="color: #3b82f6;">{}</a>', url, url.replace("https://", "")
            )
        return format_html('<span style="color: #6b7280;">{}</span>', url.replace("https://", ""))
...
class AIGenerationLogAdmin(ModelAdmin):
    ...
    def display_tokens(self, obj):
        return format_html("↓{} / ↑{}", obj.tokens_input, obj.tokens_output)
...
class ChatMessageAdmin(ModelAdmin):
    ...
    def display_role(self, obj):
        ...
        return format_html(
            '<span style="color: {}; font-weight: 500;">{} {}</span>', color, icon, obj.get_role_display()
        )
    ...
    def display_tokens(self, obj):
        if obj.tokens_used > 0:
            return format_html('<span style="font-weight: 500;">{}</span>', obj.tokens_used)
