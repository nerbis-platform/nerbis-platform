...
from .models import (
    ...
    OnboardingResponse, AIGenerationLog
)
...
class WebsiteTemplateListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'description', 'preview_image_url', 'preview_url',
...
class WebsiteTemplateDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'name', 'slug', 'industry', 'industry_display',
            'description', 'preview_url', 'structure_schema',
            'default_theme', 'is_premium', 'questions'
        ]

    def get_questions(self, obj):
        """Obtiene preguntas genéricas + específicas del template."""
        # Preguntas genéricas (template=None) + específicas del template
        generic_questions = OnboardingQuestion.objects.filter(
...
class OnboardingQuestionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'type_display', 'options', 'placeholder', 'help_text',
            'is_required', 'min_length', 'max_length', 'section', 'sort_order'
        ]
...
class BulkOnboardingResponseSerializer(serializers.Serializer):
    ...
    responses = serializers.DictField(
        ...
        help_text="Diccionario de {question_key: response_value}"
    )
    ...
    def validate_responses(self, value):
        ...
        if invalid_keys:
            raise serializers.ValidationError(
                f"Preguntas no encontradas: {', '.join(invalid_keys)}"
            )
...
class WebsiteConfigSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'ai_generations_count', 'last_generation_at',
...
class WebsiteConfigCreateSerializer(serializers.Serializer):
    ...
    template_id = serializers.IntegerField(
        help_text="ID del template seleccionado"
    )
...
class ChatRequestSerializer(serializers.Serializer):
    ...
    message = serializers.CharField(
        max_length=2000,
        help_text="Mensaje del usuario para la IA"
    )
    section_id = serializers.CharField(
        required=False,
        allow_null=True,
        help_text="ID de la sección a editar (opcional)"
    )
    context = serializers.DictField(
        required=False,
        help_text="Contexto adicional para la IA"
    )
...
class ChatResponseSerializer(serializers.Serializer):
    ...
    updated_content = serializers.JSONField(
        required=False,
        help_text="Contenido actualizado (si aplica)"
    )
    section_id = serializers.CharField(
        required=False,
        allow_null=True,
        help_text="Sección que fue modificada"
    )
    tokens_used = serializers.IntegerField(help_text="Tokens consumidos")
    remaining_generations = serializers.IntegerField(
        help_text="Generaciones restantes este mes"
    )
...
class GenerateContentRequestSerializer(serializers.Serializer):
    ...
    regenerate_section = serializers.CharField(
        required=False,
        allow_null=True,
        help_text="ID de sección específica a regenerar (null = todo el sitio)"
    )
    additional_instructions = serializers.CharField(
        required=False,
        max_length=500,
        help_text="Instrucciones adicionales para la IA"
    )
...
class GenerateContentResponseSerializer(serializers.Serializer):
    ...
    is_billable = serializers.BooleanField(
        help_text="True si se cobró como extra"
    )
...
class AIGenerationLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'section_id', 'model_used', 'tokens_input', 'tokens_output',
            'total_tokens', 'cost_estimated', 'is_successful',
...
class PublishWebsiteSerializer(serializers.Serializer):
    ...
    subdomain = serializers.SlugField(
        required=False,
        max_length=50,
        help_text="Subdominio personalizado (opcional)"
    )
    ...
    def validate_subdomain(self, value):
        ...
        if value:
            ...
            existing = WebsiteConfig.objects.filter(
                subdomain=value
