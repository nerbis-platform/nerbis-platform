...
class UsageTracker:
    ...
    @staticmethod
    def record_usage(tenant, resource, quantity=1, reference_object=None, description=""):
        """
    ...
    def check_limit(tenant, resource, additional=1):
        ...
        if resource == "employees":
            ...
            if limit is None:
                return {"allowed": True, "current": current, "limit": None, "is_over": False, "message": "Sin límite"}
            ...
            return {
                ...
                "message": f"Límite: {current}/{limit} empleados"
                if allowed
                else f"Límite alcanzado ({limit} empleados)",
            }
    ...
    def get_usage_summary(subscription):
        ...
        for resource in ["appointments", "employees", "sms", "whatsapp"]:
            if resource == "employees":
                ...
                current = StaffMember.objects.filter(tenant=subscription.tenant, is_active=True).count()
            else:
...
class SubscriptionManager:
    ...
    def cancel_subscription(subscription, at_period_end=True):
        ...
        if at_period_end:
            ...
            logger.info(
                f"Suscripción de {subscription.tenant.name} será cancelada el {subscription.current_period_end}"
            )
        else:
